...
// Window gaps only — general.gaps_in / gaps_out.
Item {
  id: root
  ...
  function gapsOut(gaps) {
    var n = root.clamp(gaps)
    return n <= 0 ? 0 : n * 2
  }

  function evalLua(gaps) {
    var n = root.clamp(gaps)
    return "hl.config({ general = { gaps_in = " + n + ", gaps_out = " + root.gapsOut(n) + " } })"
  }
  ...
  function save(gaps) {
    var n = root.clamp(gaps)
    var block = "-- BEGIN COMTROL-GAPS\n"
      + "hl.config({\n"
      + "  general = {\n"
      + "    gaps_in = " + n + ",\n"
      + "    gaps_out = " + root.gapsOut(n) + ",\n"
      + "  },\n"
      + "})\n"
      + "-- END COMTROL-GAPS\n"
    root.upsertMarker(root.looknfeelFile, "-- BEGIN COMTROL-GAPS", "-- END COMTROL-GAPS", block)
    return n
  }
